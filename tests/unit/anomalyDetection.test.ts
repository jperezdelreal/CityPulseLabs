...
// ---------------------------------------------------------------------------
// Contract types — Issue #65: Anomaly Detection — Broken Stations
// These define the API that the implementation MUST satisfy.
// Real implementation: src/services/anomalyDetection.ts
// ---------------------------------------------------------------------------
...
type AnomalyType =
  | 'never_returns_bikes'  // Empty >60 min continuously
  | 'never_empty'          // Full (no docks) >60 min continuously
  | 'zero_samples'         // No data in >24 hours
  | 'stuck_bike';          // Data hasn't changed >60 min
...
const DEFAULT_CONFIG: AnomalyCheckConfig = {
...
// Trinity replaces these with real imports from src/services/anomalyDetection.ts
// ---------------------------------------------------------------------------

function detectStaleStation(
  ...
  if (diffMinutes > thresholdMinutes) {
    // Check if values haven't changed across last N snapshots
...
function detectEmptyStation(
  ...
  // Find continuous run of 0 bikes
